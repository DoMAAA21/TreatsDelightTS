...
const MaintenancePage: FC = () => {
    ...
    const storesData: StoresData = {
        ...
        rows: stores.map((store) => ({
            _id: store._id,
            name: store.name,
            maintenance: store.maintenance ? renderMaintenanceStatus(store?.maintenance) :
            ...
            actions: (
                <div className="flex items-center  justify-center ml-6">
                    <Link to={`/admin/maintenance/store/${store._id}`} className="mr-2 w-8 h-8 md:h-12 md:w-12 lg:h-8 lg:w-8">
                        <img
                            src={ArrowRightIcon}
                            alt="Edit Icon"
